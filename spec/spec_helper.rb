...
VCR.configure do |config|
  ...
  # One Signal
  config.filter_sensitive_data('<ONE_SIGNAL_CLIENT_ID>') do
    ENV.fetch('ONE_SIGNAL_CLIENT_ID', 'some-one-signal-id')
  end
  config.filter_sensitive_data('<ONE_SIGNAL_CLIENT_KEY>') do
    Base64.decode64(ENV.fetch('ONE_SIGNAL_CLIENT_KEY', 'MjEwMDE3OWEtM2U0NS00OTEzLWIwOTEtNDcwYTUyNzcwZWU5'))
  end

  # Keepa
  config.filter_sensitive_data('<KEEPA_TEST_KEY>') do
    ENV.fetch('KEEPA_TEST_KEY', 'some-keepa-key')
  end
