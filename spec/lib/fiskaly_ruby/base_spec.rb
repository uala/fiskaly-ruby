...
RSpec.describe 'Command name to method name translation' do
  describe '.to_method_name' do
    context 'with some usual command name' do
      let(:command) { stub_const('FiskalyRuby::Some::Usual::CommandName', Class.new) }
    ...
    context 'with particular command name' do
      context 'with KassenSichV module' do
        let(:command) { stub_const('FiskalyRuby::KassenSichV::Some::CommandName', Class.new) }
      ...
      context 'with TSS module' do
        let(:command) { stub_const('FiskalyRuby::TSS::Some::CommandName', Class.new) }
      ...
      context 'with DSFinVK module' do
        let(:command) { stub_const('FiskalyRuby::DSFinVK::Some::CommandName', Class.new) }
