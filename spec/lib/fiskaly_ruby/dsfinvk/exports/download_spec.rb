...
RSpec.describe FiskalyRuby::DSFinVK::Exports::Download do
  context 'unit tests' do
    ...
    describe '#client_id' do
      it 'responds to "client_id"' do
        expect(export_download.export_id).to eq('some_export_id')
      end
    end

    describe '#call' do
      context 'with valid params' do
        let(:response) { instance_double(Net::HTTPOK, body: { foo: :bar }) }
  ...
  context 'integration tests' do
    let(:export_download) { described_class.new(token: token, export_id: export_id).call }
    let(:token) do
    ...
    context 'with existing export' do
      ...
      vcr_options = { tag: :fiskaly_service, cassette_name: 'fiskaly_service/dsfinvk/exports_download_ok' }
      it 'retrieves an export file', :aggregate_failures, vcr: vcr_options do
        expect(export_download[:status]).to eq(:ok)
        expect(JSON.parse(export_download[:body])).to match('BINARY DATA' => true)
      end
    end
    ...
    context 'with not existing export' do
      ...
      vcr_options = { tag: :fiskaly_service, cassette_name: 'fiskaly_service/dsfinvk/exports_download_not_found' }
      it 'retrieves an export file', :aggregate_failures, vcr: vcr_options do
        expect(export_download[:status]).to eq(:error)
        expect(export_download[:message]).to eq('Bad Request')
      end
    end
  end
end
